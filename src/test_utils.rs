...
pub fn spawn_main<T>(wrapped_main: T) -> (std::thread::JoinHandle<()>, std::sync::mpsc::Receiver<TestEv>) where T: FnOnce(std::os::unix::net::UnixStream) -> () + std::marker::Send + 'static{
    ...
    let jh = std::thread::spawn(move||{
        ...
        let main_join = std::thread::spawn(move||{wrapped_main(t_stdout);});
